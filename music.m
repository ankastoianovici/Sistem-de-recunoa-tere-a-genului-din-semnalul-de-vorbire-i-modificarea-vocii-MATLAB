...
close all;
clc;
[song,Fs] = audioread('piano.mp3');
Fs = Fs*4;   %Marirea freceventei
t1 = 1;      % Setează limita inferioară
t2 = 1e6;    % Setează limita superioară

% Verificare lungime
if t2 > length(song)
    t2 = length(song);
end
%Analizarea unei parti din cantec
y = song(t1:t2);
[~,n] = size(y);
t = linspace(t1,t2,n);
audiowrite('piano_window.wav',y,Fs);
...
letter
audiowrite('piano_recreated.wav',recreate_song,fs);
